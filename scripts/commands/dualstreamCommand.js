(function() {
    var otherChannels = $.getSetIniDbString('dualStreamCommand', 'otherChannels', 'Kanal-1 Kanal-2'),
    ...
    function reloadMulti() {
        ...
        interval = setInterval(function() {
            if (timerToggle && otherChannels != 'Kanal-1 Kanal-2') {
    ...
    $.bind('command', function(event) {
        ...
        if (command.equalsIgnoreCase('multi')) {
            if (!action) {
                if (otherChannels != 'Kanal-1 Kanal-2') {
                    $.say($.lang.get('dualstreamcommand.link') + $.username.resolve($.channelName) + '/' +  otherChannels.replace(' ', '/'));
                } else {
            ...
            if (action.equalsIgnoreCase('set')) {
                ...
                $.log.event(sender + ' hat Multilink festgelegt auf "' + $.lang.get('dualstreamcommand.link') + $.username.resolve($.channelName) + '/' + chan + '".');
                return;
            }
            ...
            if (action.equalsIgnoreCase('clear')) {
                ...
                $.log.event(sender + ' hat Multilink zurückgesetzt!');
                return;
            }
            ...
            if (action.equalsIgnoreCase('timerinterval')) {
                ...
                $.log.event(sender + ' hat Multi-Timer Intervall geändert zu ' + timerInterval + ' Sekunden.');
                reloadMulti();
            }
            ...
            if (action.equalsIgnoreCase('reqmessage')) {
                ...
                $.log.event(sender + ' hat Mutli-benötigte-Nachrichten geändert zu ' + reqMessages + ' Nachrichten.');
            }
        }
