...
(function() {
    ...
    function loadScript(scriptFile, force, silent) {
        if (!isModuleLoaded(scriptFile) || force) {
            ...
            try {
                ...
                if (!silent) {
                    consoleLn('Modul geladen: ' + scriptFile.replace(/\.\//g, '') + ' (' + (enabled ? 'Aktiviert' : 'Deaktiviert') + ')');
                }
            } catch (e) {
                consoleLn('Laden fehlgeschlagen "' + scriptFile + '": ' + e);
    ...
    function startInit() {
        ...
        if ($.isNightly) {
            consoleLn('PhantomBot Nightly Build - Es wird keine Unterstützung angeboten');
            consoleLn('Bitte melde Bugs einschließlich des Datums des Nightly Build und der Repo Version an:');
