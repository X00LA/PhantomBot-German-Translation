...
(function() {
    var entries = [],
        ...
        raffleMessage = $.getSetIniDbString('raffleSettings', 'raffleMessage', 'Eine Verlosung ist bereits im Gange! Tippe (keyword) um teilzunehmen. (entries) BenutzerInnen nehmen bereits teil.'),
    ...
    $.bind('command', function(event) {
        ...
        if (command.equalsIgnoreCase('raffle')) {
            ...
            if (action.equalsIgnoreCase('togglewarningmessages')) {
                ...
                $.say($.whisperPrefix(sender) + $.lang.get('rafflesystem.warningmessages.toggle') + (sendMessages ? $.lang.get('common.enabled') : $.lang.get('common.disabled')) + '.');
                return;
            }
