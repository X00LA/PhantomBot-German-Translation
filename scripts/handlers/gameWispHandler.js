...
(function() {
    ...
    var subMessage = $.getSetIniDbString('gameWispSubHandler', 'subscribeMessage', '(name) hat dich gerade auf GameWisp mit dem Rang (tier) abonniert!'),
        reSubMessage = $.getSetIniDbString('gameWispSubHandler', 'reSubscribeMessage', '(name) hat dich seit (months) Monaten in Folge via GameWisp abonniert!'),
        tierUpMessage = $.getSetIniDbString('gameWispSubHandler', 'tierUpMessage', '(name) hat seinen Rang auf (tier) hochgestuft!');
    ...
    function checkGameWispSub(username) {
        ...
        if (jsonData['data'][0]['status'].equals('inactive')) {
            ...
            $.consoleDebug('checkGameWispSub(' + username + '): Rufe Datenbank Aktualisierung ab; Mods werden ignoriert!');
            $.restoreSubscriberStatus(username, false);
