...
connection.onclose = function(data) {
    ...
    newSongAlert('WebSocket-Verbindung-gelöst', 'Starte das Panel neu, wenn der Bot wieder gestartet ist.', 'danger', 0);
    connectedToWS = false;
}
...
connection.onmessage = function(e) {
    ...
    if (messageObject['authresult'] == false) {
        if (!messageObject['authresult']) {
            newSongAlert('Websocket Authorisierung fehlgeschlagen!', 'Lade die Seite neu, wenn das fehlschlägt, dann starte den Bot neu.', 'danger', 0);
            return;
        }
        return;
    }
...
function handlePlayList(d) {
    ...
    var tableData = "<tr><th /><th>Song Titel</th><th>Dauer</th><th>YouTube ID</th></tr>";
...
function handlePlay(id, title, duration, requester) {
    ...
    try {
        ...
        newSongAlert('Spielt gerade', title, 'success', 4000);
        ...
        $("#currentSong").html("<strong>" + title + "</strong><br>" +
                               ...
                               "    <tr><td />" +
                               "        <td><div id=\"playPauseDiv\" class=\"button\" onclick=\"handlePause()\" id=\"tooltip-playPause\" data-placement=\"left\" data-toggle=\"tooltip\" title=\"Abspielen/Pause\"></div></td>" +
                               "        <td colspan=\"2\"><div id=\"songProgressBar\" data-placement=\"left\" data-toggle=\"tooltip\" title=\"Fortschritt\"></div></td>" +
                               "        <td><div id=\"skipSong\" data-placement=\"left\" data-toggle=\"tooltip\" title=\"Lied Überspringen\" class=\"button\" onclick=\"skipSong()\"><i class=\"fa fa-step-forward\" /></div></td>" +
                               "        <td><div id=\"tooltip-random\" data-placement=\"left\" data-toggle=\"tooltip\" title=\"Zufallswiedergabe An/Aus\" class=\"button\" onclick=\"randomizePlaylist()\"><i class=\"fa fa-random\" /></div></td>" +
                               "    <td /></tr>" +
                               "    <tr><td />" +
                               "        <td><div id=\"mutedDiv\" data-placement=\"left\" data-toggle=\"tooltip\" title=\"Ton aus/Ton an\" class=\"button\" onclick=\"handleMute()\"></div></td>" +
                               "        <td><div id=\"volumeControl\" data-placement=\"left\" data-toggle=\"tooltip\" title=\"Lautstärke\"></div></td>" +
                               "        <td><div id=\"search\" data-placement=\"left\" title=\"Song suchen\" class=\"button\" data-toggle=\"modal\" data-target=\"#songRequestModal\"><i class=\"fa fa-plus\" /></div></td>" +
                               "        <td><div id=\"tooltip-steal\" data-placement=\"left\" data-toggle=\"tooltip\" title=\"Song stehlen\" class=\"button\" onclick=\"stealSong()\"><i class=\"fa fa-bookmark\" /></div></td>" +
                               "        <td><div id=\"tooltip-chat\" data-placement=\"left\" data-toggle=\"tooltip\" title=\"Chat An/Aus\" class=\"button\" onclick=\"toggleChat()\"><i class=\"fa fa-comment\" /></div></td>" +
                               "    <td /></tr>" +
                               "<table>" +
...
function songRequestDiv() {
    $("#songRequestDiv").html(
        ...
        "            <div class=\"modal-title\" id=\"modalLabel\">YouTube Song-Anfrage</div>" +
        "         </div>" +
        "         <div class=\"modal-body\">" +
        "             <form role=\"form\" onkeypress=\"return event.keyCode != 13\">" +
        "                 <div class=\"form-group\">" +
        "                     <label for=\"songRequestInput\">Suchbegriff / YouTube-Link / YouTube ID</label>" +
        "                     <input type=\"text\" class=\"form-control\" id=\"songRequestInput\" placeholder=\"Song-Anfrage\" />" +
        "                 </div>" +
        "             </form>" +
        "         </div>" +
        "         <div class=\"modal-footer\">" +
        "             <button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\" onclick=\"getFormSongRequest()\">Übermitteln</button>" +
