...
(function() {
        ...
        if (panelHasQuery(msgObject)) {
            if (panelCheckQuery(msgObject, 'keywords_keywords')) {
                if (msgObject['results'].length === 0) {
                    $('#keywordsList').html('<i>Keine Schlüsselwörter definiert!</i>');
                    return;
                }
            ...
            if (panelCheckQuery(msgObject, 'keywords_cooldown')) {
                if (msgObject['results'].length === 0) {
                    $('#keywordsCooldownList').html('<i>Keine Schlüsselwörter-Verzögerung definiert!</i>');
                    return;
                }
            ...
            if (panelCheckQuery(msgObject, 'keywords_price')) {
                if (msgObject['results'].length === 0) {
                    $('#keywordsPriceList').html('<i>Kein Schlüsselwort-Preis definiert!</i>');
                    return;
                }
    ...
    function addKeyword() {
        ...
        if (keyword.length > 0 && response.length > 0) {
            $('#addKeywordInput').val('Übermittle...');
            $('#addKeywordResponseInput').val('Übermittle...');
    ...
    function addKeyCooldown() {
        ...
        if (input.length > 0 && command.length != 0) {
            ...
            $("#cooldownKeyInputCommand").val("Übermittelt");
            setTimeout(function() { doQuery(); }, TIMEOUT_WAIT_TIME);
    ...
    setInterval(function() {
        ...
        if (active == 11 && isConnected && !isInputFocus()) {
            newPanelAlert('Aktualisiere Schlüsselwort-Daten...', 'success', 1000);
            doQuery();
        }
    }, 3e4);
