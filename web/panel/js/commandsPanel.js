...
(function() {
    ...
    function onMessage(message) {
        ...
        if (panelHasQuery(msgObject)) {
            ...
            if (panelCheckQuery(msgObject, 'commands_aliases')) {
                if (msgObject['results'].length === 0) {
                    $('#aliasCommandsList').html('<i>Es sind keine benutzerdefinierten Alias-Befehle definiert.</i>');
                    return;
                }
            ...
            if (panelCheckQuery(msgObject, 'commands_pricecom')) {
                if (msgObject['results'].length === 0) {
                    $('#priceCommandsList').html('<i>Es sind keine Befehle mit Preisen definiert.</i>');
                    return;
                }
                msgObject['results'].sort(sortCommandTable);
                for (idx in msgObject['results']) {
    ...
    function addCustomCommand() {
        ...
        if (command.length == 0) {
            $('#addCommandCommand').val('[FEHLER] Bitte definiere einen Wert!');
            $('#addCommandText').val('');
            setTimeout(function() { $('#addCommandCommand').val(''); }, TIMEOUT_WAIT_TIME * 10);
            return;
        } else if (commandText.length == 0) {
            $('#addCommandText').val('[FEHLER] Bitte definiere einen Wert!');
            $('#addCommandCommand').val('');
            setTimeout(function() { $('#addCommandText').val(''); }, TIMEOUT_WAIT_TIME * 10);
            return;
        } else if (command.match(/\s+/)) {
            $('#addCommandCommand').val('[FEHLER] Dein Befehl darf kein Leerzeichen enthalten.');
            $('#addCommandText').val('');
            setTimeout(function() { $('#addCommandCommand').val(''); }, TIMEOUT_WAIT_TIME * 10);
            return;
        } else if (botCommands.indexOf(command) !== -1) {
            $('#addCommandCommand').val('[FEHLER] Befehl existiert bereits.');
            $('#addCommandText').val('');
            setTimeout(function() { $('#addCommandCommand').val(''); }, TIMEOUT_WAIT_TIME * 10);
            return;
        }
    ...
    function aliasCommand() {
        ...
        if (alias.length == 0) {
            $("#aliasCommandInputAlias").val("[FEHLER] Bitte definiere einen Wert!");
    ...
    function addCooldown() {
        ...
        if (command.match(/\s/)) {
            $("#cooldownCmdInputCommand").val("Abklingzeiten dürfen keine Leerzeichen enthalten, bitte lege die Abklingzeit für den Standardbefehl fest.");
    ...
    setInterval(function() {
        ...
        if (active == 1 && isConnected && !isInputFocus()) {
            newPanelAlert('Aktualisiere Befehls-Daten...', 'success', 1000);
            doQuery();
        }
    }, 3e4);
