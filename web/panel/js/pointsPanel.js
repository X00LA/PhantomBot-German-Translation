...
(function() {
    ...
    function onMessage(message) {
        var msgObject,
            groupPointKeys = [ "Streamer", "Administrator", "Moderator", "Abonnent", "Spender", "Stammzuschauer", "Zuschauer" ];
            timezone = "CET"; // Default time zone in Core if none given.
        ...
        if (panelHasQuery(msgObject)) {
            ...
            if (panelCheckQuery(msgObject, 'points_grouppoints')) {
                ...
                for (key in groupPointKeys) {
                    ...
                    if (groupPoints === '-1') {
                        html += "<td style=\"float: right\"><i>Verwendet globalen Wert</i></td>";
                    } else {
                        html += "<td />";
                    }
                    html += "</tr>";
    ...
    function setPointName() {
        ...
        if (singleName.match(/\s/ig) || pluralName.match(/\s/ig)) {
            $("#setPointsNameInput").val("Dein Punktename darf kein Leerzeichen enthalten.");
    ...
    function setPointGain(action) {
        ...
        if (value.indexOf('.') !== -1 || (parseInt(value) < 1 && parseInt(value) !== 0)) {
            $("#setPointGainInput_" + action).val('Nur natürliche Zahlen sind erlaubt.');
